...
// Clean existing receipts and transactions to allow re-run
  ...
  const accounts = await prisma.account.findMany({ select: { id: true, type: true, accountType: true } });
  const accountMap = Object.fromEntries(accounts.map((a: any) => [a.accountType?.trim().toLowerCase(), a.id]));
